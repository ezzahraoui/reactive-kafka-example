...
class CommittableSourceConsumerActor extends Actor with ActorLogging {
  ...
  override def postStop() = {
    log.info("Shutdown actor")
  }

  def receive = {
    case "start" =>
      Consumer.committableSource(consumerSettings, Subscriptions.assignment(new TopicPartition("topic1", 1)))
        .mapAsync(1) { msg =>
          log.info(s"CommittableSourceConsumer consume: $msg")
          msg.committableOffset.commitScaladsl()
        }
        .runWith(Sink.ignore).recoverWith {
          case e =>
